...
dl_config_shell() {
    ...
    curl -s --connect-timeout 3 https://raw.githubusercontent.com/qg888/Notes/main/QL/config.sample.sh > $config_shell_path
...
dl_extra_shell() {
    ...
    curl -s --connect-timeout 3 https://raw.githubusercontent.com/qg888/Notes/main/QL/extra.sh > $extra_shell_path
...
dl_code_shell() {
    ...
    curl -s --connect-timeout 3 https://raw.githubusercontent.com/qg888/Notes/main/QL/code.sh > $code_shell_path
...
dl_task_before_shell() {
    ...
    curl -s --connect-timeout 3 https://raw.githubusercontent.com/qg888/Notes/main/QL/task_before.sh > $task_before_shell_path
...
add_curl_sample() {
    if [ "$(grep -c "config.sample.sh" /ql/config/crontab.list)" != 0 ]; then
        ...
        # 获取token
        token=$(cat /ql/config/auth.json | jq --raw-output .token)
        curl -s -H 'Accept: application/json' -H "Authorization: Bearer $token" -H 'Content-Type: application/json;charset=UTF-8' -H 'Accept-Language: zh-CN,zh;q=0.9' --data-binary '{"name":"自动更新模板","command":"curl -L https://raw.githubusercontent.com/qg888/Notes/main/QL/config.sample.sh -o /ql/sample/config.sample.sh && cp -rf /ql/sample/config.sample.sh /ql/config","schedule":"45 6,18 * * *"}' --compressed 'http://127.0.0.1:5700/api/crons?t=1627380635389'
    fi
}
run_curl_sample() {
    curl -sL https://raw.githubusercontent.com/qg888/Notes/main/QL/config.sample.sh -o /ql/sample/config.sample.sh && cp -rf /ql/sample/config.sample.sh /ql/config
}
if [ "${all}" = 1 ]; then
